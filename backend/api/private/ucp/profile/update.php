<?php
/* 
  Edit profile endpoint (mysqli)
  - Uses $conn from /backend/headerBackend.php
  - Validates name/email/phone/org fields
...
if ($password !== '') {
    ...
    if (defined('PASSWORD_ARGON2ID')) {
        $passwordHash = password_hash(
            $password,
            PASSWORD_ARGON2ID,
            ['time_cost' => 4, 'memory_cost' => 1 << 17, 'threads' => 2]
        );
    } else {
...
try {
    $query = 'UPDATE users 
              SET email = ?, full_name = ?, phone = ?, org_type = ?, org_name = ?, country = ?, city = ?';
